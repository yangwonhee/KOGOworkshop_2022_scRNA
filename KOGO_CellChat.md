...
``` r
# good at visualization, cellphoneDB대신 사용함. 나온지 얼마 안된 툴.
# cell cell communication / human, rat 
library(devtools)
library(Seurat)
library(scater)
library(harmony)
...
``` r
covid <- rownames(meta)[meta$group == 'S/C'] # cell information, condition별로 
covid_input = expr[, covid]
covid_meta = meta[covid, ]
unique(covid_meta$celltype) # 필수로 넣어야하는 info
```
