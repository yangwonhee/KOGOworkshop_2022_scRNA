...
``` r
# seuart 은 단순하게 normalization, scran umi 분포를 clustering해서 size factor(과정이 좀더 복잡)
# cell specific한 vious를 , 특정 celldpsms 100개 1000개 다를 수 있기 때문에..
clusters <- quickCluster(sce)
sce <- computeSumFactors(sce, clusters = clusters) # cluster기반 size factor 계산
...
``` r
# cell별 variance, mean gen expression과 variance가 차이가 없어야되지만,, 
# 실제로 유의미한 varience만 보겠다는 것.
...
``` r
hvg.norm <- getTopHVGs(gene_variance, fdr.threshold = 0.05) # highly variable gene을 구할 수 있음.
length(hvg.norm) 
```
...
seurat <- as.Seurat(sce.norm,
                    counts = "counts",
                    data = "logcounts") # seurat object로 변환. 계산한 count format으로 바뀜.
